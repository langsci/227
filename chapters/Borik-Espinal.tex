...
\begin{document}
	...
	In contrast to \ili{Russian}, in a language with overt \isi{determiners}, like for instance \ili{English}, the subject of a sentence corresponding to \REF{ex:borik:1a} will be expressed by means of a \is{definite generics}definite generic \citep{Carlson1977} or the singular generic \citep{Chierchia1998} \textit{the} N construction (i.e. \textit{the panda}), as in \REF{ex:borik:2a}. On the other hand, \ili{English} also allows bare plurals to refer to kinds, as illustrated in \REF{ex:borik:2b}.
	...
	Note, however, that theoretically, there could still be an option that while in \ili{English} the kind referring \is{determiner phrases}DP has to be definite, in \ili{Russian} it might be \is{indefinites}indefinite. Let us next discuss why this is not the case.
	...
	Finally, a pronominal substitution test also shows that definite kinds behave like DPs rather than other, ``smaller'' types of arguments. The test as used in \citet{Pereltsvaig2006} shows that third person pronouns can be used to substitute full DPs, but not \is{quantifier phrases}QPs or NPs, which can only be substituted by other (\is{quantifiers}quantificational and/or pronominal) elements. The example below (based on \citealt[446, ex. 15a]{Pereltsvaig2006}) shows that a pronominal subject of \REF{ex:borik:20b} can only substitute the agreeing subject of \REF{ex:borik:20a}. 
	...
	Modified kinds are \is{kind reference}ind-referring expressions composed by a \is{nouns}noun and a modifier, normally expressed by an \is{adjectives}adjective, provide an additional semantic argument for the \isi{definiteness} of \ili{Russian} \is{bare nominals}bare nominal kinds. Consider the data in\largerpage \REF{ex:borik:26}.

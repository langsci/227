...
\citet{Roberts2003} argues that definites conventionally trigger two presuppositions: one of \is{weak familiarity|(}weak familiarity, and a second one called \is{informational uniqueness}\textsc{informational uniqueness}. These are the informational counterparts of Russellian existence and uniqueness, respectively. 
...
\begin{exe}
\exi{\fts{(i)}}\label{ex:etxeberria:i} \textnormal{Basque\il{Basque}}
	\begin{xlist}
	\exi{\fts{a.}}[]{ \footnotesize
		\gll {ikasle-a}\\
		\fts{student-\textsc{det.sg}}\\
		\glt \fts{{`the student'}}}
	\exi{\fts{b.}}[]{ \footnotesize
		\gll {ikasle} {hau/hori/hura}\\
		\fts{student} \fts{\textsc{dem.sg.proximal/medial/distal}}\\
		\glt \fts{{`this/that/that student'}}}
	\exi{\fts{c.}}[*]{ \footnotesize 
		\gll {ikasle-a} {hau/hori/hura}\\
		\fts{student-\textsc{det.sg}} \fts{\textsc{dem.sg.proximal/medial/distal}}\\
		\glt \fts{{(`this/that/that student')}}}
	\end{xlist}
\end{exe}} \newpage 
...
The definite reduplication phenomenon only happens with D; the \is{indefinite articles}indefinite article results in ungrammaticality: \largerpage[3]

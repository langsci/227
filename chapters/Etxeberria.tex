...
Notice that, contrary to all other approaches, for \citet{CoppockBeaver2015} \textit{the} is a \is{non-saturating functions}non-saturated constituent in the referential use. We come back to this assumption later. We take it here that the use of D creates a morphologically definite argument, it is thus the core of what can be understood as ``\isi{definiteness}''.
...
Basque\il{Basque} \citep[(37)]{etxeberria2005}
...
Our overall conclusion is that ``\isi{definiteness}'' is a family of phenomena revealing the following functions of D: 
...
Our D plus \is{quantificational determiners|)}Q data remain mysterious under this analysis. (Also mysterious remain \is{weak definites}weak definite data where \isi{uniqueness} appears to be systematically violated). Roberts's theory of \isi{definiteness}, on the other hand, seems to provide a more appropriate frame for \isi{domain restriction}. 

\citet{Roberts2003} argues that definites conventionally trigger two presuppositions: one of \is{weak familiarity|(}weak familiarity, and a second one called \is{informational uniqueness}\textit{informational uniqueness}. These are the informational counterparts of Russellian\ia{Russell, Bertrand} existence and uniqueness, respectively. 
...
\ea\label{ex:etxeberria:23}
	...
	\ex
		{Semantic \is{definiteness}Definiteness:} A \is{determiner phrases}DP is definite if it carries an \is{anaphora}anaphoric presupposition of weak familiarity.
	\ex
...
A third argument against the DP analysis comes from Basque\il{Basque}, where it is possible to conjoin two NPs or two APs under the same single D, as shown below (in \ili{Greek} this is not possible, so we cannot apply this test).
...
\ea[*]{ \textit{every the woman} ; *\textit{kathe i gynaika}}
\z

Having D\textsubscript{DR} as an NP modifier is consistent with the idea of a lower \is{determiner phrases|)}DP layer, as we mentioned earlier (see \citealt{szabolcsi1987}, \citealt{szabolcsi2010}, and works cited in \citealt{AlexiadouEtAlii2008}). If \il{Salish!St'\'at'imcets Salish}St'\'at'imcets D is D\textsubscript{DR}, the \ili{Salish} structures are not as peculiar as initially appearing, but illustrate a systematic \isi{grammaticalization} of \isi{domain restriction} via D. However, D on NP is generally not allowed in \ili{English}, \ili{Greek} and Basque\il{Basque}:
...
Our take of this idea is that one D is referential, the other(s) perform D\textsubscript{DR}. While the D plus NP introduces a referent, the additional D combining with \is{adjectives}adjectives performs \isi{domain restriction}, and the multi-D structure akin to a partitive.
...
\ea{\label{ex:etxeberria:46} \ili{Greek} \\
	\gll {Idame} {tis} {dilitiriodis} {\op}\textnormal{\#}{tis}{\cp} {kobres}.\\
	saw.\textsc{1pl} the poisonous the cobras\\
	\glt `We saw the poisonous cobras.' \citep{kolliakou2004}}
\z

\ea{\label{ex:etxeberria:47} \ili{Greek} \\
	\gll \llap{\#~}{Tous} {epikindinous} {tous} {kakopious} {prepi} {na} {tous} {apofevgeis}.\\
	the dangerous the criminals must subj them avoid\\
	\glt `You must avoid the dangerous criminals.' \citep{CamposStravrou2004}}
\z
...
\glt `Did you meet the beautiful singer?'\\ \largerpage
...
For this analysis to work, a number of assumptions must be made. First, we need to assume \is{definiteness}definiteness ``concord'' (\textit{\`a la} \citealt{zeijlstra2004}); but there is no explanation why reduplication is optional whereas concord is obligatory. And a concord analysis would render the difference between a monadic and a polydefinite vacuous semantically, missing the \is{partitives}partitive and anti-\isi{uniqueness} effects observed, as well as the correlation with the impossibility of the partitive with \is{adjectives}adjectives that we noted. The concord/apposition account, finally, fails to unify reduplication with the D on \is{quantificational determiners}Q. 

Our analysis does precisely that. It unifies definite reduplication with the D\textsubscript{DR} strategy on a predicate and says that \is{polydefinites|)}polydefinites fall under the phenomenon of \isi{domain restriction}, which involves a modifier function of D.  In turns out, then, very interestingly, that \ili{Greek} has both options of D\textsubscript{DR}. Two open questions are: (a) why Basque\il{Basque} doesn't exhibit the D-reduplication strategy, and (b) whether our D\textsubscript{DR} analysis can extend to capture D-reduplication in other languages (e.g. in \ili{Swedish}, noted earlier). We will leave the latter as a prediction of our theory, to be tested in future research. 

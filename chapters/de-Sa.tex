...
\abstract{We discuss the properties of \textsc{weak definite} noun phrases, \isi{definite noun phrases} (henceforth DP) in which the DP does not uniquely refer to an individual referent. Since one of the properties of \isi{generic noun phrases} is that they do not uniquely refer, we asked whether weak definites might in fact be a form of generic noun phrase. We adopted a quantitative and \is{experimental study}experimental approach conducting a corpus analysis and four experiments that were designed to assess whether weak definites differ that properties of DP that are generic, weak and regular definites. A corpus analysis by \citet{deSaEtAlii2016} showed that generic DP and weak definites are not in complementary distribution. And a follow-up analysis on verb \is{aktionsarten}\textit{aktionsart} showed that most weak definite appear in telic or activity DP. The experiments also compared matched sentences with weak, regular and generic reading DPs. These studies do not find similarities between weak definites and \isi{generics}. We conclude that weak definite noun phrases are not generics.
...
\begin{enumerate}
\item If weak definites are in fact \isi{generics},  then generic DP and weak definites should either occur in the same environments, or if they do not, should be in complementary distribution with one another, indicating that they are variations of the same linguistic type. \newpage
...
The \is{morphosyntax}morphosyntactic choices for \isi{generics} was an interesting property especially the use of \is{bare nouns}bare noun forms, which have a generic reading. The final experiment used a forced completion task to investigate the forms that repetition would take.
